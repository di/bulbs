...
from bulbs.utils import json, build_path, get_file_path, coerce_id


# The default URIs
ORIENTDB_URI = "http://localhost:2480/"

# The logger defined in Config
log = get_logger(__name__)

# OrientDB resource paths
...
class OrientDBResponse(Response):
    ...
    :ivar results: A generator of OrientDBResult objects, a single
        OrientDBResult object, or None, depending on the number of
        results returned.
    :ivar total_size: The number of results returned.
    :ivar raw: Raw HTTP response. Only set when log_level is DEBUG.

    """
    result_class = OrientDBResult
    ...
    def get_results(self):
        ...
        :return:  A tuple containing two items: 1. Either a
            generator of OrientDBResult objects, a single
            OrientDBResult object, or None, depending on the number
            of results returned; 2. An int representing the number
            results returned.
        :rtype: tuple

        """
        if isinstance(self.content.get('results'), list):
            results = (self.result_class(result, self.config)
...
class OrientDBRequest(Request):

    """Makes HTTP requests to OrientDB and returns a OrientDBResponse."""

    response_class = OrientDBResponse


class OrientDBClient(Client):

    """
    Low-level client that sends a request to OrientDB and returns a response.
    ...
    :ivar request: OrientDBRequest object.

    Example:

    >>> from bulbs.orientdb import OrientDBClient
    ...
    def _get_uri(self, db_name):
        if db_name is not None:
            uri = "http://localhost:2480/database/%s" % db_name
            return uri
    ...
    def create_edge(self, outV, label, inV, data={}, keys=None):
        ...
        if keys or self.config.autoindex is True:
            ...
            return self.create_indexed_edge(
                outV, label, inV, data, index_name, keys=keys)
        data = self._remove_null_values(data)
    ...
    def get_or_create_vertex_index(self, index_name, index_params=None):
        ...
        resp.results = OrientDBResult(result, self.config)
        return resp
...
#    return self.gremlin(script)
    ...
    def put_vertex(self, index_name, key, value, _id):
        ...
        """
        # OrientDB's API only supports string lookups so convert value to a
        # string
        path = build_path(index_path, index_name)
        params = {
            'key': key,
    ...
    def remove_vertex(self, index_name, _id, key=None, value=None):
        ...
        """
        # Can OrientDB have None for key and value?
        path = build_path(index_path, index_name)
    ...
    def create_indexed_edge(
            self, outV, label, inV, data, index_name, keys=None):
        """
        ...
        :param keys: Property keys to index. Defaults to None
            (indexes all properties).
        :type keys: list
    ...
    def _remove_null_values(self, data):
        ...
        clean_data = [(k, data[k])
                      for k in data if data[k] is not None]  # Python 3
